      ...
      {pageNavLinks.map((item) => (
        <div key={item.name} className="group">
          <NavLink
            to={item.path}
            className={({ isActive }) =>
              `relative inline-flex items-center px-1 text-xs leading-[62px] font-medium ${
                isActive
          ...
          {/* Dropdown Menu */}
          {item.dropdown && (
            <div className="pointer-events-none invisible absolute left-0 mt-0 w-full border-t border-gray-100 bg-white opacity-0 shadow-lg transition-all duration-200 ease-out group-hover:pointer-events-auto group-hover:visible group-hover:opacity-100">
              <div className="p-8">
                <div className="flex max-h-[60vh] gap-5 overflow-hidden">
                  ...
                  {/* Categories Section */}
                  <div className="grid w-full grid-cols-3 gap-8 overflow-y-auto pr-4 pl-4">
                    {/* Men's Section */}
